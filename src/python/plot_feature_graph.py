...
def PlotGraph(
    ...
    edge_width,
    font_size,
    edge_width_exponent=args.edge_width_exponent,
    colors=None,
    special=0,
):
    color = "C5"
    ...
    widths = [
        (abs(el) / max([abs(eli) for eli in widths])) ** edge_width_exponent
        * edge_width
        * sign(el)
        for el in widths
    ]
    ...
    if colors == None:
        ...
        colors = MakeColors(
            [el for el in weights], minima_=-1, maxima_=1, colormap="RdBu"
        )
        edges = nx.draw_networkx_edges(
            G,
            pos=pos,
