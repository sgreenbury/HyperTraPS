...
def WriteOutRoutes(rout, ofile):
    ...
    df_out_routes.to_csv(
        outfile.replace("edge-list" + node_tag, "routes"),
        index=None,
        header=None,
        sep=" ",
    )
...
def MakeGraph(df, L):
    ...
    for node in G.nodes():
        ...
        for edge in G.out_edges(node):
            ...
            G.edges[edge]["inv_lcprob"] = -np.log(
                G.edges[edge]["weight"] / tweight
            )
    return G, nt
...
def CalculateAngle(
    v, G, pos, direction_weight=args.direction_weight, node_type=args.node_type
):
    ones = v.count(1)
...
def AdjustGroupPos(pos, L, gpos, gamma=0.0, gap_type="uniform"):
    ...
    for key in gpos:
        ...
        if len(el2) > 1:
            ...
            hammings = [
                Hamming(el2[i + 1][-2], el2[i][-2]) for i in range(len(dists))
            ]
            ...
            if gap_type == "uniform":
                ...
                dists2 = np.array(
                    [(1 - gamma) * el + gamma * gap for el in dists]
                )
                ...
                if gap_type == "hamming":
                    ...
                    for i, el in enumerate(dists):
                        _el = (1 - gamma) * el + gamma * Hamming(
                            el2[i + 1][-2], el2[i][-2]
                        ) * (max_d - min_d) / hamming_sum
                        dists2.append(_el)
                    dists2 = (
                        np.array(dists2) / np.sum(dists2) * (max_d - min_d)
                    )
...
def CalculateNodeSizes(
    ...
    m = max(node_sizes)
    return [
        ((el / m) ** node_scale_exponent) * node_scale for el in node_sizes
    ]
...
def Noverlap(
    ...
    for key in gpos:
        ...
        if len(el2) < 2:
            continue
        adjustments[key] = (
            (amax_d - amin_d) * (1 - space_to_diam) / np.sum(diams)
        )

    min_adjustment = min(adjustments.values())
    node_sizes_adjust = {}
    for node in node_sizes:
        node_sizes_adjust[node] = (
            (node_sizes[node] ** 0.5) * min_adjustment / 2
        )
...
def MakeTopSizesByOnes(G, node_sizes, node_type=args.node_type):
    ...
    for key in top_sizes:
        top_sizes[key] = sorted(
            top_sizes[key].items(), key=operator.itemgetter(1), reverse=True
        )
    return top_sizes
...
def MakeProbableLabelSet(
    ...
    paths = k_shortest_paths(
        G=G, source=source, target=target, k=n_r - 1, weight="inv_lcprob"
    )
    labels = {}
    big_labels = []
    for path in paths:
...
def MakeProbableLabelSetData(
    ...
    for i in range(int(len(df) / 2)):
        ...
        if source not in G.nodes() or target not in G.nodes():
            continue
        try:
            path = k_shortest_paths(
                G=G, source=source, target=target, k=0, weight="inv_lcprob"
            )
        except:
            continue
        paths.append(path[0])
