...
fig.set_size_inches(
    args.width,
    args.height if args.aspect == None else args.width * args.aspect,
)
ax1 = fig.add_subplot(111)
ax1.set_xlabel("Parameters", labelpad=-0.1)
ax1.set_ylabel("AIC")
...
if args.f2 != None:
    po_fo_aic = ax1.scatter(
        min_k_fo,
        min_aic_fo,
        color="C1",
        label="First order regularised",
        s=ms,
        marker="o",
        zorder=zm,
        alpha=alpha_point,
...
po_so_aic = ax1.scatter(
    min_k_so,
    min_aic_so,
    color="C2",
    label="Second order regularised",
    s=ms,
    marker="o",
    zorder=zm,
    alpha=alpha_point,
...
if args.f2 != None:
    p_fo_aic = ax1.plot(
        ...
        color="C1",
        linestyle=":",
        label="First order regularisation",
        zorder=0,
    )
p_so_aic = ax1.plot(
    params_so,
    ...
    color="C2",
    linestyle=":",
    label="Second order regularisation",
    zorder=0,
)
...
bb = [
    sp.left,
    sp.top + bbox_extra * (sp.top - sp.bottom),
    sp.right - sp.left,
    bbox_extra * (sp.top - sp.bottom),
]
...
if args.pdf == "no":
    fig.savefig(
        out_file + ".png", dpi=600, bbox_inches="tight", transparent=True
    )
