...
def MakePdfComplete(df, normalise=args.normalise):
    ...
    pdf = np.array(
        [
            (el / totals[i] if totals[i] != 0 else el)
...
def EarliestAndLatest(in_file=args.transition_data, l=L):
    ...
    if in_file != None:
        ...
        df = pd.read_csv(in_file, header=None, index_col=None, sep=" ").astype(
            int
        )
        for i in range(0, len(df), 2):
            ...
            for j in range(len(df.iloc[i, :])):
                if df.iloc[i + 1, j] != df.iloc[i, j]:
                    out[j][0] = (
                        ones_start if ones_start < out[j][0] else out[j][0]
                    )
                    out[j][1] = (
                        ones_end if ones_end + 1 > out[j][1] else out[j][1]
                    )
    return out
