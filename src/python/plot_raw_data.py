...
args.outfile = (
    (args.f).replace(".txt", "_") + args.plot_type + "." + args.outfile_type
)
...
if args.order == "horizontal":
    if args.plot_type == "spy":
        ...
        if args.xticklabels == "yes":
            ...
            a2 = list(
                np.arange(
                    args.xlabelevery - 0.5, df.shape[1] + 0.5, args.xlabelevery
                )
            )
            ax.set_xticks(np.array(a1 + a2))
            xticklabels = [
                (str(int(i) - 1) if i > 1 else str(int(i)))
                for i in (np.arange(1, df.shape[1] + 1.1, args.xlabelevery))
            ]
        ...
        for i in range(df.shape[0] + 1):
            ax.fill_between(
                [0, df.shape[1] + 1], i - ygap, i + ygap, color="white", lw=0
            )
        for i in range(df.shape[1] + 1):
            ax.fill_betweenx(
                [0, df.shape[0] + 1], i - xgap, i + xgap, color="white", lw=0
            )
